...
if LIVE:
    ...
    dest_dir = "/net/data/dns-ttl/parent"
    dest_dir_apache = "/home/protick/ocsp_dns_django/ttldict/logs_final_v2/apache_1/apache2/"
...
def bind_transfer():
    send_msg("Parent: Starting bind transfer")

    files_before_log_rotate = [join(bind_dir, f) for f in listdir(bind_dir) if isfile(join(bind_dir, f))]

    send_msg("Parent: Files before:" + concat_str(files_before_log_rotate))
    ...
    send_msg("Parent: Files after logrotate:" + concat_str(files_after_log_rotate))
    ...
    for file in files_to_transfer:
        ...
        execute_cmd(cmd)
        msg_str = "Parent: moved {} to {}, size {} MB".format(file.split("/")[-1], file_name, file_size_in_mb)
        send_msg(msg_str)
    ...
    send_msg("Parent: Files after sending:" + concat_str(files_at_end))
...
scheduler.add_job(bind_transfer, 'interval', minutes=10)
# scheduler.add_job(apache_transfer, 'interval', minutes=15)
scheduler.start()